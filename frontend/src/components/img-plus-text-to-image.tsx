...
const ImagePlusTextToImageShowcase: React.FC<Props> = ({ baseURL }) => {
  const [genStatus, setGenStatus] = useState<string>()
  const [spinner, setSpinner] = useState<boolean>(false)
  const [base64Image, setBase64Image] = useState<any>(sareeImage);
  const [results, setResults] = useState<any>([]);
  const [txtInput,setTxtInput] = useState<string>('silk');
  ...
  function search() {
    ...
    fetch(
      baseURL + "search/image",
      {
        ...
        body: JSON.stringify({ imageInput: payload, textInput: txtInput}),
      }
    )
      .then((res) => res.json())
      .then((data) => {
          ...
          <Card className='Card-display'>
            ...
            <Card.Header>
              ...
              <Accordion className='Change-product' flush>
                ...
                <input type="text" id="txt" placeholder="Specify cotton, brand, etc."  value={txtInput}
                      onChange={(e) => {
                      if(e.target?.value){
...
export default ImagePlusTextToImageShowcase;
